...
const Chat = () => {
  ...
  return (
    <div className="chat-container">
      ...
      {messages.map((msg) => (
  <div key={msg.id} className={`chat-msg ${msg.sender === 'future-me' ? 'future-me' : ''}`}>
    <span>{msg.message}</span>
      ...
      <div className="input-container">
        ...
        <div className={`input-field ${isSwitchOn ? '' : 'orange-focus'}`}>
          ...
          />
          <button onClick={handleSendMessage} className={newMessage.trim() === '' ? 'button-disabled' : ''} disabled={!newMessage.trim()}>Send</button>
        </div>
      </div>
    </div>
  );
